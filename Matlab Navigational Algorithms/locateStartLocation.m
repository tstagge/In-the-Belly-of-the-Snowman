...
%     posTemp = empty struct template for a Robot Position Struct
% OUT:
%     centers = the rounded! average coordinates for each contiguous group
%               of cells (in a list of point structs)

function center = locateStartLocation(map, size, mapO, tolCont, posTemp)
%function centers = locateBeacons(map, size, tolCont, posTemp)
    %----LOCATE ALL 40s IN MAP---------------------------------------------
    all40s = [];
    %all40s = repmat(struct(posTemp),1,1000);
    for i = 1:1:size(1)
        for j = 1:1:size(2)
            if((map(i,j) == 40) && not(i == mapO.x) && not(j == (size(1) - mapO.y)))
                pt = struct(posTemp);
                ...
                if(contiguousX & contiguousY) %Contiguous; need to add element
                    ...
                    if(contGroupSizes(group) >= contGroupSizeMax) %Have to extend every row
                        ...
                        for j = 1:contNumGroups %Creates a column array of the proper size to tack on
                            emptyAppendArray = [emptyAppendArray; struct(posTemp)];
                        end
...
%             end
        ...
        if(foundContiguous == false)%Not contiguous with any current group/row; must add a new row
            ...
            for k = 1:contGroupSizeMax
                emptyAppendArrayRow = [emptyAppendArrayRow, struct(posTemp)];
            end
    ...
    %----FIND CENTER OF CONTIGUOUS BLOCK-----------------------------
...
%         avgY(g) = sumY(g) / contGroupSizes(g);
    ...
    switch(numCenters)
        case 0
            fprintf('ERROR: No start location found.\n');
        case 1
            %Peachy -- found one start location
        otherwise
            fprintf('ERROR: Multiple start locations found.\n');
            for i = 1:numCenters
                fprintf('\tLoc %d: (%d, %d)', i, centers(i).x, centers(j).y);
            end
    end
    center = centers(1);
